...
\ifprof
\begin{corrige}
Dans la mesure ou le nombre de données d'entrées et le nombre de données d'entrainement semblent faibles, l'utilisation d'un réseau de neurones ne se justifie pas forcément. 

Comme avantage, on peut noter la facilité pour sa mise en oeuvre. 

Comme inconvénient, on peut noter la difficulté pour choisir les différents paramètres (nombre de couches et nombre de neurones par couche).
...
\ifprof
\begin{corrige}
Dans la mesure où les données d'entrainement sont étiquétes, l'apprentissage supervisé est donc naturel.
\end{corrige}
\else
\fi
...
\ifprof
\begin{corrige}
On a $f(x)=(1+e^{-x})^{-1}$ et $f'(x)=(-1)\left(-e^{-x}\right)(1+e^{-x})^{-2} = \dfrac{e^{-x}}{\left(1+e^{-x}\right)^2}$.
\begin{lstlisting}
def f_prime(x) :
